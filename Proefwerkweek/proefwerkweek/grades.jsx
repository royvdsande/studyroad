...
function fmtNL(n, d = 1) { return n == null ? "—" : n.toFixed(d).replace(".", ","); }
...
const NAAM2KEY = {
  "Bedrijfseconomie": "beco", "Biologie": "biologie", "Engels": "engels", "Frans": "frans",
  "Natuurkunde": "natuurkunde", "Nederlands": "nederlands", "Scheikunde": "scheikunde", "Wiskunde B": "wiskundeb",
};

function progTag(p) {
  if (!p) return null;
  if (p.type === "veilig") return `✓ ${p.waarde}`;
...
function GradeRow({ vak, index = 0 }) {
  ...
  const isHD = vak.toetsen.length > 0 && numeriek.length === 0;
    ...
    <div className="gr-card" data-open={open} style={{ "--c": vak.kleur, animationDelay: (index * 0.04) + "s" }}>
      <button className="gr-main" onClick={() => setOpen((v) => !v)}>
        ...
        <span className="gr-chips">
          {numeriek.length > 0 ? numeriek.map((t, i) => (
            <span className="gchip" key={i} data-fail={t.n < 5.5}><b>{t.cijfer}</b><small>{t.soort}</small></span>
          )) : <span style={{ fontSize: 13, color: "var(--ink-soft)" }}>{isHD ? `${vak.toetsen.length}× afgerond · handelingsdeel` : "—"}</span>}
        </span>
        <span className="gr-avg">
          {gem == null ? <span className="avg-num none">{isHD ? "voldaan" : "—"}</span> : <span className={"avg-num " + klasse}>{vak.gemiddelde}</span>}
        ...
        <div className="gr-detail">
          ...
          {vak.toetsen.length > 0 && (
            <table className="gr-table">
              <thead><tr><th>Toets</th><th>Type</th><th>Weging</th><th>Datum</th><th style={{ textAlign: "right" }}>Cijfer</th></tr></thead>
              <tbody>
                {vak.toetsen.map((t, i) => (
                  <tr key={i}>
    ...
    <div className="page section-pad">
      <div className="kpis">
        ...
        <div className="kpi">
          ...
          <div className="v" style={{ color: "var(--pass)" }}>{voldoende}<small>/{cijferVakken.length}</small></div>
...
Object.assign(window, { CijfersView });
